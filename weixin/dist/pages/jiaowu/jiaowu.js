...
/***/ function(module, exports, __webpack_require__) {
	...
	var DataRepository = function () {
	    function DataRepository() {
	        (0, _classCallCheck3.default)(this, DataRepository);
	    }

	    (0, _createClass3.default)(DataRepository, null, [{
	        key: 'addData',
	        ...
	        value: function addData(data) {
	            ...
	            data['_id'] = (0, _util.guid)();
	            return DataRepository.findAllData().then(function (allData) {
	                allData = allData || [];
	                allData.unshift(data);
	                wx.setStorage({ key: _Config2.default.ITEMS_SAVE_KEY, data: allData });
	            });
	        }
	        ...
	        value: function removeData(id) {
	            return DataRepository.findAllData().then(function (data) {
	                if (!data) return;
	                ...
	                wx.setStorage({ key: _Config2.default.ITEMS_SAVE_KEY, data: data });
	            });
	        }
	        ...
	        value: function removeRange(range) {
	            ...
	            return DataRepository.findAllData().then(function (data) {
	                if (!data) return;
	                var indexs = [];
	                for (var rIdx = 0, rLen = range.length; rIdx < rLen; rIdx++) {
	                    for (var idx = 0, len = data.length; idx < len; idx++) {
	        ...
	        value: function saveData(data) {
	            ...
	            return DataRepository.findAllData().then(function (allData) {
	                ...
	                for (var idx = 0, len = allData.length; i < len; i++) {
	        ...
	        value: function findAllData() {
	            return (0, _util.promiseHandle)(wx.getStorage, { key: _Config2.default.ITEMS_SAVE_KEY }).then(function (res) {
...
/***/ function(module, exports, __webpack_require__) {
	...
	var _DataRepository = __webpack_require__(74);

	var _DataRepository2 = _interopRequireDefault(_DataRepository);
	...
	var DataSerivce = function () {
	    function DataSerivce(props) {
	        (0, _classCallCheck3.default)(this, DataSerivce);
	    ...
	    (0, _createClass3.default)(DataSerivce, [{
	        key: 'save',
	        value: function save() {
	            if (this._checkProps()) {
	                return _DataRepository2.default.addData({
	                    title: this.title,
	        ...
	        value: function findAll() {
	            return _DataRepository2.default.findAllData().then(function (data) {
	        ...
	        value: function findById(id) {
	            return _DataRepository2.default.findBy(function (item) {
	                return item['_id'] == id;
	            }).then(function (items) {
	        ...
	        key: 'deleteRange',
	        value: function deleteRange(ids) {
	            return _DataRepository2.default.removeRange(ids);
	        }
	        ...
	        value: function findByDate(date) {
	            if (!date) return [];
	            return _DataRepository2.default.findBy(function (item) {
	                return item && item['date'] == date.getDate() && item['month'] == date.getMonth() && item['year'] == date.getFullYear();
	            }).then(function (data) {
	                return data;
	            });
	        }
...
/***/ function(module, exports, __webpack_require__) {
	...
	var _DataService = __webpack_require__(80);
	...
	Page({
	  data: {
	    ...
	    //事项等级数据
	    levelSelectedValue: _Config.LEVEL.normal,
	    levelSelectData: [_Config.LEVEL.normal, _Config.LEVEL.warning, _Config.LEVEL.danger],
	  ...
	  onLoad: function onLoad() {
	    var _this = this;
	    (0, _util.promiseHandle)(wx.getSystemInfo).then(function (data) {
	      _this.setData({
	  ...
	  dateClickEvent: function dateClickEvent(e) {
	    var _e$currentTarget$data2 = e.currentTarget.dataset,
	        year = _e$currentTarget$data2.year,
	        month = _e$currentTarget$data2.month,
	        date = _e$currentTarget$data2.date;
	  ...
	  showUpdatePanelEvent: function showUpdatePanelEvent() {
	    showUpdatePanel.call(this);
	    this.setData({ isEditMode: true });
	  },
	  closeUpdatePanelEvent: function closeUpdatePanelEvent() {
	    closeUpdatePanel.call(this);
	  },
	  editClickEvent: function editClickEvent() {
	    this.setData({ isEditMode: true });
	  },


	  // 事项列表项长按动作事件
	  listItemLongTapEvent: function listItemLongTapEvent(e) {
	    ...
	    if (!isEditMode) {
	      var itemList = ['详情', '删除'];
	      (0, _util.promiseHandle)(wx.showActionSheet, { itemList: itemList, itemColor: '#2E2E3B' }).then(function (res) {
	        if (!res.cancel) {
	          switch (itemList[res.tapIndex]) {
	            case '详情':
	              wx.navigateTo({ url: '../jiaowu_detail/jiaowu_detail?id=' + id });
	              break;
	            case '删除':
	              new _DataService2.default({ _id: id }).delete().then(function () {
	  ...
	  //取消编辑事件
	  cancelEditClickEvent: function cancelEditClickEvent() {
	  ...
	  // 事项标题文本框变化事件
	  todoInputChangeEvent: function todoInputChangeEvent(e) {
	  ...
	  //事项内容多行文本域变化事件
	  todoTextAreaChangeEvent: function todoTextAreaChangeEvent(e) {
	    var value = e.detail.value;
	  ...
	  // 选择事项等级事件  
	  levelClickEvent: function levelClickEvent(e) {
	  ...
	  saveDataEvent: function saveDataEvent() {
	    ...
	    var _data = this.data,
	        todoInputValue = _data.todoInputValue,
	        todoTextAreaValue = _data.todoTextAreaValue,
	        levelSelectedValue = _data.levelSelectedValue;
	    var _data$data$selected = this.data.data.selected,
	        year = _data$data$selected.year,
	        month = _data$data$selected.month,
	        date = _data$data$selected.date;

	    if (todoInputValue !== '') {

	      var promise = new _DataService2.default({
	  ...
	  //批量删除事件
	  removeRangeTapEvent: function removeRangeTapEvent() {
	    ...
	    wx.showModal({
	      ...
	      success: function success(res) {
	        if (res.confirm) {
	          _DataService2.default.deleteRange(_this.data.editItemList).then(function () {
	            loadItemListData.call(_this);
	          });
	          _this.setData({
	            editItemList: [],
	  ...
	  listItemClickEvent: function listItemClickEvent(e) {
	    var isEditMode = this.data.isEditMode;
	    var id = e.currentTarget.dataset.id;
	    ...
	    var editItemList = this.data.editItemList || [];
	    ...
	    if (index >= 0) {
	      ...
	      var tIndx = editItemList.findIndex(function (item) {
	        return item == id;
	      });
	...
	function loadItemListData() {
	  var _data$data$selected2 = this.data.data.selected,
	      year = _data$data$selected2.year,
	      month = _data$data$selected2.month,
	      date = _data$data$selected2.date;

	  var _this = this;
	  _DataService2.default.findByDate(new Date(Date.parse([year, month, date].join('-')))).then(function (data) {
	...
	function changeDate(targetDate) {
	  ...
	  showDate = void 0,
	      //当前显示第几天
	  showMonthFirstDateDay = void 0,
	      //当前显示月份第一天的星期
	  showMonthLastDateDay = void 0,
	      //当前显示月份最后一天的星期
	  showMonthDateCount = void 0; //当前月份的总天数
	  ...
	  //获取上一页的显示天数
	  if (showMonthFirstDateDay != 0) beforeDayCount = showMonthFirstDateDay - 1;else beforeDayCount = 6;

	  //获取下页的显示天数
	  if (showMonthLastDateDay != 0) afterDayCount = 7 - showMonthLastDateDay;else showMonthLastDateDay = 0;

	  //如果天数不够6行，则补充完整
	  var tDay = showMonthDateCount + beforeDayCount + afterDayCount;
	  if (tDay <= 35) afterDayCount += 42 - tDay; //6行7列 = 42

	  var selected = this.data.data['selected'] || { year: showYear, month: showMonth, date: showDate };
	  var selectDateText = selected.year + '年' + (0, _util.formatNumber)(selected.month) + '月' + (0, _util.formatNumber)(selected.date) + '日';
	  ...
	  if (beforeDayCount > 0) {
	    ...
	    for (var fIdx = 0; fIdx < beforeDayCount; fIdx++) {
	      dates.unshift({
	        _id: _id,
	        year: beforeYear,
	        month: beforMonth,
	  ...
	  for (var cIdx = 1; cIdx <= showMonthDateCount; cIdx++) {
	    dates.push({
	      _id: _id,
	  ...
	  if (afterDayCount > 0) {
	    for (var lIdx = 1; lIdx <= afterDayCount; lIdx++) {
	      dates.push({
	        _id: _id,
	        year: afterYear,
	        month: afterMonth,
