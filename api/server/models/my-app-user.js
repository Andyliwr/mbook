...
module.exports = function (Myappuser) {
  ...
  Myappuser.remoteMethod("fundBackPwd", {
    accepts: {
      arg: "email",
      type: "string",
    },
  ...
  Myappuser.sayHi = function (callback) {
    //定义一个http接口方法
    Myappuser.find({ username: "user" })
      .then(function (res) {
        console.log(res);
        callback(null, "hi");
      })
  ...
  //处理微信登录
  Myappuser.getSessionId = function (
    wxcode,
    userInfo,
    rawData,
    signature,
    ...
    ep.all("hasFinishedWx", function (returnData) {
      ...
      if (true || signature2 == signature) {
        ...
        checkRegisteEp.all("hasFinishedCheck", function (checkData) {
          console.log("Debug: Myappuser.getSessionId -> checkData", checkData);
          if (checkData) {
            if (checkData.code == -1) {
            ...
            cb(null, {
              code: -1,
              errmsg: "查询myappuser失败，未找到和当前openid对应的用户",
            });
          }
        });
        ...
        Myappuser.find({ auth: openidReg }, function (err, res) {
          ...
          if (thisUserArr.length) {
            ...
            console.log(
              "用户openid: " + returnData.data.openid + " 未绑定myappuser账号"
            );
            checkData = {
              code: 0,
              isRegisted: 0,
  ...
  Myappuser.remoteMethod("getSessionId", {
    accepts: [
      {
        arg: "wxcode",
        type: "string",
  ...
  Myappuser.remoteMethod("checkSessionId", {
    accepts: {
      arg: "sessionid",
      type: "string",
  ...
  Myappuser.remoteMethod("getUploadToken", {
    accepts: {
      arg: "key",
      type: "string",
  ...
  Myappuser.getMyBooks = function (userid, cb) {
    Myappuser.findById(userid, { myBooks: 1, _id: 0 }, {}, function (err, res) {
      ...
      allMyBooks.forEach(function (item, index) {
        app.models.factionlists.findById(
          item.bookid,
          { id: 0, sectionArray: 0 },
          {},
          function (err, res) {
  ...
  Myappuser.remoteMethod("getMyBooks", {
    accepts: {
      arg: "userid",
      type: "string",
      description: "用户id",
  ...
  Myappuser.addMyBooks = function (userid, bookids, cb) {
    ...
    if (typeof userid === "string" && typeof bookids === "string") {
      ...
      getBookDetailEp.after("hasFinishedDetail", bookidArr.length, function (
        ...
        Myappuser.findById(userid, { id: 0, sectionArray: 0 }, function (
          err,
          res
        ) {
          if (err || !res) {
            ...
            Myappuser.update({ id: userid }, { myBooks: finalArr }, function (
              err,
              res
            ) {
              if (err) {
  ...
  Myappuser.remoteMethod("addMyBooks", {
    accepts: [
      {
        arg: "userid",
        type: "string",
  ...
  Myappuser.remoteMethod("deleteMyBooks", {
    accepts: [
      {
        arg: "userid",
        type: "string",
  ...
  //用户退出阅读器之前更新已阅读章节
  Myappuser.updateHasRead = function (userid, bookid, hasRead, cb) {
    //定义一个http接口方法
    Myappuser.findById(userid, { id: 0, sectionArray: 0 }, function (err, res) {
      if (err || !res) {
        ...
        if (hasThisBook === false) {
          ...
          Myappuser.update({ id: userid }, { myBooks: finalMyBooks }, function (
            err,
            res
          ) {
            if (err || !res) {
  ...
  Myappuser.remoteMethod("getUserInfo", {
    accepts: {
      arg: "userid",
      type: "string",
      description: "用户id",
  ...
  Myappuser.remoteMethod("updateUserInfo", {
    accepts: [
      {
        arg: "userid",
        type: "string",
  ...
  Myappuser.remoteMethod("countReadTime", {
    accepts: [
      {
        arg: "userid",
        type: "string",
  ...
  Myappuser.remoteMethod("continueReadDay", {
    accepts: {
      arg: "userid",
      type: "string",
      description: "用户id",
  ...
  Myappuser.remoteMethod("getAllUser", {
    returns: [{ arg: "data", type: "string" }],
    http: {
      verb: "get",
      path: "/getAllUser",
