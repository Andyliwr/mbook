...
var APPID = "wx39635e80215f2e13";
var SECRET = "fdb3ef6c9a40e724b39d19fd830c4937";
//上传个人头像到七牛云服务器
var qiniu = require("qiniu");
var QINIU_BUCKET = "andyliwr-server-song"; //上传空间名
qiniu.conf.ACCESS_KEY = "dFVk3VZuYURIG2clGaXIep9awYrpzILprxHtX1ZP";
qiniu.conf.SECRET_KEY = "z8zfwNwIFo_Uy6w5gKLHjYvubafTgCuBNAhUXiY1";
...
module.exports = function (user) {
  ...
  user.getSessionId = function (
    ...
    ep.all("hasFinishedWx", function (returnData) {
      ...
      if (true || signature2 == signature) {
          ...
          if (checkData) {
            if (checkData.code == -1) {
              ...
              if (checkData.isRegisted == 0) {
                ...
                var city = (
                  (userData.country == "CN" ? "China" : userData.country) +
                  " " +
                  userData.province +
                  " " +
                  userData.city
                ...
                var keyValue =
                  "{session_key: " +
                  checkData.session_key +
                  ", openid: " +
                  checkData.openid +
                  "}";
                console.log(
                  "sessionid: " + sessionid + ", keyValue: " + keyValue
                );
            ...
            cb(null, {
              code: -1,
              errmsg: "查询user失败，未找到和当前openid对应的用户",
            });
          }
        });
        ...
        user.find({ auth: openidReg }, function (err, res) {
          var checkData = null;
          if (err) {
            console.log("查询user失败，" + err);
            checkData = { code: -1, errMsg: "查询user失败，" + err };
          ...
          if (thisUserArr.length) {
            console.log(
              "openid为" +
                JSON.parse(thisUserArr[0].auth).wxOpenId +
                "的用户已经绑定了user的账号" +
                thisUserArr[0].username +
                ", userId为" +
                thisUserArr[0].id +
                "..."
            );
            ...
            console.log(
              "用户openid: " + returnData.data.openid + " 未绑定user账号"
            );
            checkData = {
              code: 0,
              isRegisted: 0,
    ...
    // request(
    //   "https://api.weixin.qq.com/sns/jscode2session?" + content,
    ...
    //         returnData = {
    //           code: 0,
    //           data: { session_key: wxdata.session_key, openid: wxdata.openid },
    //         };
    //       } else {
    //         returnData = {
    //           code: -1,
    //           errMsg:
    //             "使用code交换openid和session_key接口返回失败" + wxdata.errmsg,
    //         };
    //       }
    //     } else {
    //       returnData = {
    ...
    //   }
    // );
    ep.emit("hasFinishedWx", {
      code: 0,
      data: { session_key: "xxx", openid: "asdas1231231231" },
    });
  };
  user.remoteMethod("getSessionId", {
    accepts: [
      {
        arg: "wxcode",
        type: "string",
  ...
  user.remoteMethod("checkSessionId", {
    accepts: {
      arg: "sessionid",
      type: "string",
  ...
  //获取用户所有的书籍
  user.getMyBooks = function (userid, cb) {
    user.findById(userid, { myBooks: 1, _id: 0 }, {}, function (err, res) {
      if (err || !res) {
          ...
          function (err, res) {
            ...
            if (err || !res) {
              ...
              if (urlReg.test(headImage)) {
                headImage =
                  "http:" + headImage.substring(0, headImage.length - 2);
              }
              getBookDetailEp.emit("hasFinishedDetail", {
                success: 1,
                index: index,
            ...
            // app.models.chapter.find({res: resReg}, {limit: 1, sort: {sectionNum: 1}}, function(contentErr, contentRes){
  ...
  user.addMyBooks = function (userid, bookids, cb) {
    //拿到书籍的详细信息
  ...
  //删除书架里的书籍
  user.deleteMyBooks = function (userid, bookids, cb) {
    //拿到书籍的详细信息
    if (typeof userid === "string" && typeof bookids === "string") {
      var bookidArr = bookids.split(",");
      //对传入的数据做验证，只有booklist中存在的id才是被加进来
  ...
  user.remoteMethod("deleteMyBooks", {
    accepts: [
      {
        arg: "userid",
        type: "string",
  ...
  //用户退出阅读器之前更新已阅读章节
  user.updateHasRead = function (userid, bookid, hasRead, cb) {
    //定义一个http接口方法
  ...
  user.getUserInfo = function (userid, cb) {
    user
      .findById(userid)
      .then(function (res) {
        if (res) {
          var returnData = {
            ...
            avatar:
              res.avatar ||
              "https://file.lantingshucheng.com/myApp/unknown_headimg.png?imageView2/1/w/60/h/60/format/jpg/interlace/1/q/75|imageslim",
            gender: res.gender,
            realm: res.realm || "",
  ...
  // 更新个人信息
  user.updateUserInfo = function (userid, info, cb) {
    user
      .findById(userid)
      .then(function (res) {
        if (res) {
  ...
  // 统计阅读时长
  user.countReadTime = function (userid, addtime, cb) {
    user
      .findById(userid)
      .then(function (res) {
        if (res) {
  ...
  user.remoteMethod("countReadTime", {
    accepts: [
      {
        arg: "userid",
        type: "string",
  ...
  // 统计累计阅读天数
  user.continueReadDay = function (userid, cb) {
    user
      .findById(userid)
      .then(function (res) {
        if (res) {
  ...
  user.remoteMethod("continueReadDay", {
    accepts: {
      arg: "userid",
      type: "string",
      description: "用户id",
  ...
  user.getAllUser = function (cb) {
    user
      .find({ fields: { id: true, username: true, nickName: true } })
      .then(function (res) {
        if (res) {
