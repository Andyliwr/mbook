...
function updateEveryBook(index, book, total) {
  return new Promise((resolve, reject) => {
    try {
      logger.debug(`正在进行第${index}本书籍《${book.factionName}》的更新，总共有${total}本...`);
      if (book.source && book.source instanceof Array) {
        ...
        book.source.forEach((item, index) => {
          sub1Quene.add(async () => {
            // 暂停10s
            await delay(10000);
            const chapters = await getSourceData(item, book.newest);
          ...
          chapters.forEach((chapter, index) => {
            sub2Queue.add(async () => {
              ...
              if (!oldChapter) {
                ...
                logger.debug('已经更新<' + book.factionName + '>最新章节数字');
              } else {
